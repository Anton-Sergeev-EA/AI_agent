"""
Модуль основного класса ассистента.
"""
import time
import re
import random
...
class IntelligentAssistant:
    ...
    def _init_command_patterns(self) -> Dict[str, Callable]:
        ...
        return {
            r'(привет|здравствуй|доброе утро|добрый день|добрый вечер|hi|hello)': self._handle_greeting,
    ...
    def process_command(self, command: str) -> str:
        ...
        # ===== ПРОВЕРКА ШУТОК (В САМОМ НАЧАЛЕ) =====
        if command:
            command_clean = command.strip().lower()
            if any(word in command_clean for word in ['шутка', 'шутку', 'пошути', 'рассмеши', 'анекдот']):
                jokes = [
    ...
    def _handle_greeting(self, command: str) -> str:
        ...
        greetings = [
            f"Привет! Я {self.name}, ваш цифровой помощник. Чем могу помочь?",
            f"Здравствуйте! Я {self.name}. Чем могу быть полезен?",
            f"Приветствую! Я {self.name}, всегда готов помочь!",
            f"Добрый день! Это {self.name}. Слушаю вас!"
        ]
...
"""
    ...
    def _handle_unknown(self, command: str) -> str:
        # Проверяем, не является ли команда математическим выражением (без ключевых слов)
        if re.search(r'[\d][\s]*[\+\-\*\/\^][\s]*[\d]', command):
            result = self.skills['calculator'].process(command)
